...
#        self.database.createStream(7, 2, 27.5, 2, 71, t1)
    ...
    def RealTimeInfoOper(self, newServerSocket, RoomNumber):
        ...
        while True:
            ...
            newServerSocket.recv(1024)
            print("即将发送的TempSet信息：", self.TemperatureSet[RoomNumber])
            ...
            print(self.Temperature[RoomNumber])
            if self.virtualClock % 3 == 0:
                for i in range(self.RoomCount):
                    # self.CostRecord(i) -----------------------------------------------这里为了测试，注释掉了写入数据库的函数--------------------
                    pass
            time.sleep(2)
    ...
    def Menu(self, newServerSocket2, RoomNumber):  # 与客户端的Menu对应
        ...
        if choose == "1":
            ...
            # print("选择了修改模式,,修改后的模式为：", self.Mode[RoomNumber])
            self.TemperatureSet[RoomNumber] = 25
        elif choose == "4":
            ...
            print("室温发生了修改")
            Temp = newServerSocket2.recv(1024).decode()
            self.Temperature[RoomNumber] = int(Temp)
